...
const SketchClipboard = ({
  ...
  const drawFrame = useCallback((rc, ctx, canvas) => {
    ...
    const padding = 0.8;
    const scale =
      (Math.min(canvas.width, canvas.height) / originalSize) * padding;

    const xOffset =
      (canvas.width - originalSize * scale) / 2;
    const yOffset =
      (canvas.height - originalSize * scale) / 2;
  ...
  useSketchFrameCache(canvasRef, drawFrame, [width, height, fps], { fps });

  return (
    <div className="relative inline-flex items-center">
      {/* Copied Text */}
      <span
        className={`absolute left-full ml-2 top-1/2 -translate-y-1/2 whitespace-nowrap text-sm font-bold transition-all duration-300 ${
          copied
