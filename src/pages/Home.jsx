...
const Home = () => {
  ...
  const handleCreateRoom = async () => {
    try {
      ...
      if (response.roomCode) {
        ...
        logger(CONFIG.fileName, CONFIG.methods.handleCreateRoom, CONFIG.messages.createRoomSuccess, response);
      }
    } catch (err) {
      showErrorToast(CONFIG.messages.createRoomFailure)
      logger(CONFIG.fileName, CONFIG.methods.handleCreateRoom, CONFIG.messages.createRoomFailure, err);
  ...
  try {
    ...
    if (response.success) {
      ...
      const userIsReconnecting = players.some(player => player.username === username);
      ...
      if (response.room.status === CONFIG.roomStatus.PLAYING && !userIsReconnecting) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-3">
      ...
      <div className="text-md md:text-xl lg:text-2xl mb-8 font-gloria text-center w-full max-w-2xl">
